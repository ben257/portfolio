...
// --- fix 1: robust mouse → canvas coords (works with CSS scaling / 100% width)
function getMousePosInCanvas(evt, canvas){
...
function updateMouse(event) {
    const pos = getMousePosInCanvas(event, canvas);
    mouse.x = pos.x;
    mouse.y = pos.y;

    // unchanged logic calls:
...
const radios = form.elements['fractalType'];

function toggleCoefficients() {
if (form.fractalType.value === 'julia') {
    coefficients.style.display = 'block';
} else {
...
radio.addEventListener('change', toggleCoefficients);
});
...
form.addEventListener('submit', (e) => {
  ...
  const fractal_type = formData.get('fractalType');
  if (fractal_type === 'julia') {
    c = new Complex(parseFloat(obj["realCoefficient"].replace(/−/g, '-')),parseFloat(obj["imaginaryCoefficient"].replace(/−/g, '-')))
    console.log(c);
  }
  else {
    c = null;
